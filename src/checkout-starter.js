...
//const [cardElement, getCardInfo] = useState(0);
    ...
    return (
        <Formik
            ...
            onSubmit={async (values, actions) => {
                ...
                console.log('payment_intent', resp.data.payment_intent)
                // calls stripe.confirmCardPayment
                const result = await stripe.confirmCardPayment(resp.data.payment_intent, {
                    payment_method: {
        ...
        >{form => (
            <PaymentForm form={form} total={total} con={context} hist={history} />
        )}</Formik>
    )
}
